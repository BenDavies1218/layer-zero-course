# 🎮 LayerZero OApp Challenges

Welcome to the LayerZero Challenge Arena! Test your cross-chain skills through various missions that will take you from novice messenger boy to an omnichain master.

Each challenge focuses on one or more of the lessons. Some challenges involve exploiting vulnerable Oapp contracts, while others require building solutions to problems.

---

Use of Claude, GPT, Cursor or other AI to complete the Challenges is absolutely discouraged. You will probably be required to research Solidity or other Smart Contract information, please reference any sources in your solutions.

## 🏆 Challenges

| Challenge                                                       | Difficulty |
| --------------------------------------------------------------- | ---------- |
| [Chain Whisperer](./challenges/challenge-01-chain-whisperer.md) | 🟢 Easy    |
| [Quantum Thief](./challenges/challenge-02-quantum-thief.md)     | 🟢 Easy    |
| [Cosmic Voting](./challenges/challenge-03-cosmic-voting.md)     | 🟡 Medium  |
| [Bridge Breaker](./challenges/challenge-04-bridge-breaker.md)   | 🟡 Medium  |
| [Nexus Prime Bank](./challenges/challenge-05-nexus-prime.md)    | 🔴 Hard    |

---

### 🟢 [Challenge 1: The Chain Whisperer 🌌](./challenges/challenge-01-chain-whisperer.md)

Master the ancient "Omnichain Echo Stone" by building a messenger that transforms messages as they travel between chains. Send "hello" and receive "🔮 [Arbitrum] HELLO WORLD" back!

**Skills**: Basic OApp, Message Transformation, Auto-Response

[**Start Challenge →**](./challenges/challenge-01-chain-whisperer.md)

---

### 🟢 [Challenge 2: The Quantum Thief 💎](./challenges/challenge-02-quantum-thief.md)

A developer claims their cross-chain vault is "unbreakable" and left thousands of precious Labrys tokens as a challenge. Find the exploit and drain their vault's across all chains.

**Already Deployed**: Vulnerable contracts on 3 testnets waiting to be exploited.

[**Start Challenge →**](./challenges/challenge-02-quantum-thief.md)

### 🟡 [Challenge 3: Cosmic Council Voting 🗳️](./challenges/challenge-03-cosmic-voting.md)

Build an omnichain governance portal where council members scattered across 2 blockchain realms can vote on proposals. Implement vote aggregation, deadline enforcement, and prevent double-voting across chains.

**Skills**: ABA Pattern, Multichain Broadcast, State Aggregation, Governance

[**Start Challenge →**](./challenges/challenge-03-cosmic-voting.md)

---

### 🟡 [Challenge 4: Bridge Breaker 🔓](./challenges/challenge-04-bridge-breaker.md)

BridgeCorp deployed their "audited and secure" bridge with 15,000 Labrys tokens across 3 chains. They're offering a job to whoever can drain it and explain how. Two vulnerabilities await discovery!

**Skills**: Security Auditing

[**Start Challenge →**](./challenges/challenge-04-bridge-breaker.md)

---

### 🔴 [Challenge 5: Nexus Prime - The Omnichain DeFi Bank 👑](./challenges/challenge-05-nexus-prime.md)

**The Great Convergence** is here. Build a production-grade omnichain DeFi bank that unites treasuries across all blockchain realms!

**Your Mission**:

- Omnichain deposit system on **(Minimum 2 EVM chains + Pre-deployed Solana Oapp)**
- Withdraw on **any chain** (deposit on solana, withdraw on Base!)
- Maintain perfect TVL accounting across all realms

**Skills**: ALL previous lessons, Solana Integration, Production-Grade Security

[**Start Challenge →**](./challenges/challenge-05-nexus-prime.md)

---

_Last Updated: November 2025_

_Course Version: 1.0_

_Course Founder: Ben Davies_
